...
namespace ariel {
    ...
    public:
        ...
        static bool isCyclicUtil(size_t v, std::vector<bool>& visited, std::vector<bool>& recStack, std::vector<size_t>& parent, const Graph& grp, std::vector<size_t>& cycle);
        static void relaxEdges(const Graph& grp, std::vector<int>& dist, std::vector<size_t>& pred);
        static std::string reconstructPath(size_t start, size_t end, const std::vector<size_t>& pred);
